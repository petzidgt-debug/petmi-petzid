...
/* FILTERS */
.filters{background:#fff;border-bottom:1px solid #eee;padding:12px 20px;display:flex;align-items:center;gap:10px;flex-wrap:wrap;position:sticky;top:60px;z-index:90}
.filter-btn{padding:8px 16px;border-radius:24px;border:1.5px solid #ddd;font-size:13px;font-weight:600;color:#888;cursor:pointer;background:#fff;transition:all .15s;white-space:nowrap}
.filter-btn.active{border-color:#00B4B4;background:#e0f7f7;color:#007a7a}
.search-input{flex:1;min-width:180px;padding:8px 14px;border:1.5px solid #e0e0e0;border-radius:24px;font-size:13px;outline:none}
.search-input:focus{border-color:#00B4B4}
.results-count{font-size:13px;color:#888;margin-left:auto;white-space:nowrap}
...
<!-- HERO -->
<div class="hero">
  <h1>La comunidad <span>PetzID</span></h1>
  <p>Conoce a todas las mascotas de la familia PetMi</p>
  <div class="hero-stats">
    <div class="hero-stat"><div class="hero-stat-num" id="statTotal">-</div><div class="hero-stat-lbl">Mascotas</div></div>
    <div class="hero-stat"><div class="hero-stat-num" id="statPerros">-</div><div class="hero-stat-lbl">Perros</div></div>
    <div class="hero-stat"><div class="hero-stat-num" id="statGatos">-</div><div class="hero-stat-lbl">Gatos</div></div>
    <div class="hero-stat"><div class="hero-stat-num" id="statAngeles">-</div><div class="hero-stat-lbl">Angelitos 🐾</div></div>
  </div>
</div>
...
<!-- FOOTER -->
<div class="footer">
  <div class="footer-brand">petz<span>ID</span></div>
  <div class="footer-sub">PetMi Guatemala — Comunidad de mascotas</div>
</div>
...
<div class="login-modal-overlay" id="loginOverlay">
  <div class="login-modal" style="position:relative">
    ...
    <div class="login-sub">Ingresa tu correo registrado para ver y editar tus mascotas.</div>
    <div class="login-msg" id="loginMsg"></div>
    <input type="email" class="login-input" id="loginEmail" placeholder="tu@correo.com" inputmode="email">
    <button class="btn-login" id="btnLogin" onclick="verificarEmail()">Ingresar</button>
...
<script>
// ── Estado global ─────────────────────────────────────────────
var allPets=[], filtered=[], shown=0, perPage=40, currentFilter='todos';
...
function renderCumpleaneros(){
  ...
  var html=cumpleMes.map(function(p){
    ...
    var hoy=new Date().getMonth()===mesActual;
    var fechaLabel=p.tipoFecha==='llegada'?'Llegó a casa':'Cumpleaños';
    ...
    return '<div class="cumple-card">'
      ...
      +'<div class="cumple-fecha">'+fechaLabel+'</div>'
...
function renderAngeles(){
  ...
  var html=angeles.map(function(p){
    ...
    var fechaPartida=p.fechaAngelito?new Date(p.fechaAngelito).toLocaleDateString('es-GT',{day:'2-digit',month:'short',year:'numeric'}):'';
...
function renderGallery(append){
  ...
  var html=toShow.map(function(p,i){
    ...
    return '<div class="pet-card" onclick="openModal('+idx+')">'
      ...
      +'<div><span class="pet-especie '+(esGato?'gato':'perro')+'">'+(esGato?'Gato':'Perro')+'</span>'
      +'</div></div></div>';
  }).join('');
  if(!append){
...
function openModal(idx){
  var p=filtered[idx];
  if(!p)return;
  // Angelitos abren su pagina de perfil para mensajes publicos
  if(p.angelito){
    window.location.href='/perfil.html?uid='+p.uid;
    return;
  }
  _renderModal(p);
}
...
function _renderModal(p){
  ...
  if(fechaLbl&&fechaVal){
    fechaLbl.textContent=p.tipoFecha==='llegada'?'Llegó a casa':'Nacimiento';
...
function renderModalFooter(p){
  ...
  if(esMia){
    footer.innerHTML='<div style="background:#e0f7f7;border-radius:10px;padding:10px 14px;font-size:12px;color:#007a7a;font-weight:500;width:100%;text-align:center">✏️ Para editar o gestionar a <strong>'+p.nombre+'</strong>, ve a <a href="/familia.html" style="color:#007a7a;font-weight:700">Mi Familia</a></div>';
    return;
  }
  ...
  if(!sessionEmail){
    footer.innerHTML=
      '<button class="btn-primary btn-pink" style="flex:1" onclick="openLoginModal()">🔑 Ingresar</button>'+
...
function agregarOtra(){window.location.href='/index.html?agregar=1';}
...
function toggleAngel(esAngel){
  ...
  document.getElementById('angelFechaInput').value=new Date().toISOString().split('T')[0];
}
...
function verificarEmail(){
  ...
  if(!email||email.indexOf('@')<0){mostrarMsgLogin('Por favor ingresa un correo valido','error');return;}
  ...
  fetch('/api/galeria?action=checkEmail',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'checkEmail',email:email})})
    ...
    .then(function(d){
      if(d.found&&d.mascotas&&d.mascotas.length>0){
        sessionEmail=email;
        sessionDueno=d.mascotas[0].dueno||'';
        localStorage.setItem('petzid_email',sessionEmail);
        localStorage.setItem('petzid_dueno',sessionDueno);
        mostrarMsgLogin('¡Bienvenido/a '+(sessionDueno||'')+'! '+d.mascotas.length+' mascota(s) encontrada(s).','success');
        setTimeout(function(){
          closeLoginModal();
          shown=0;applyFilter();
        ...
        mostrarMsgLogin('No encontramos ese correo. ¿Quieres registrarte?','error');
        btn.disabled=false;btn.textContent='Ingresar';
      }
    })
    .catch(function(){
      mostrarMsgLogin('Error de conexion. Intenta de nuevo.','error');
      btn.disabled=false;btn.textContent='Ingresar';
    });
}
...
async function publicarPerdidoEnActividades(p, ubicacion, telefono, mensaje, fecha, recompensa, posterURL){
  ...
  // Subir imagen del poster a Cloudinary
  var imagenURL = '';
  try {
    var fd = new FormData();
    fd.append('file', posterURL);
    fd.append('upload_preset', 'petzid');
    fd.append('folder', 'petzid_perdidos');
    var res = await fetch('https://api.cloudinary.com/v1_1/dh5wgqgmk/image/upload', {method:'POST', body:fd});
    var data = await res.json();
    imagenURL = data.secure_url || '';
  } catch(e){ console.log('Cloudinary poster:', e); }
...
<button class="fab-pill" style="background:#00B4B4" onclick="irACrear()"><i class="ti ti-plus" aria-hidden="true"></i>Crear PetzID</button>
<script>
function irACrear(){
  var email=localStorage.getItem('petzid_email');
  if(email) window.location.href='/index.html?agregar=1';
